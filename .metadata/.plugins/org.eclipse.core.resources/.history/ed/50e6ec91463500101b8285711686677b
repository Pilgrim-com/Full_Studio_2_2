...
typedef enum {
	STATUS_HOME = 1 << 0,  // 00001 = 1
	STATUS_JOG = 1 << 1,  // 00010 = 2
	STATUS_POINT = 1 << 2,  // 00100 = 4
	STATUS_GO_TO_TARGET = 1 << 3,  // 01000 = 8
	STATUS_STOP = 1 << 4   // 10000 = 16
} MovingStatusFlags;
...
float32_t A_f32_revolute[16] = { 1.0000, 0.0009, -0.0040, 0.0, 0.0, 0.8425,
		-7.8627, 0.0066, 0, 0, 1.0000, 0, 0.0, -0.0333, 0.2928, -0.0003 };

float32_t B_f32_revolute[4] = { 0.0001, 0.1143, 0, 0.9049 };
...
void set_moving_status(MovingStatusFlags new_status) {
	...
	registerFrame[0x10].U16 = new_status;
}
/* USER CODE END 4 */
